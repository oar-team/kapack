{ stdenv, fetchFromGitHub, cmake, elfutils, perl, python3, boost, valgrind, bash
...
stdenv.mkDerivation rec {
  ...
  preCheck = ''
       ...
       # catch by patchShebangs utility
       find ./teshsuite -type f | xargs sed -i -e 's#\(.*\)\w/bin/sh\(.*\)#\1${bash}/bin/bash#'
    '';
  checkPhase = ''
    runHook preCheck
